...
(() => {
  const {
    $, $all, show, fmtCode, countdown,
    setCurrentRoom, setRole
  } = window.Common;
  const ioSocket = window.Common.socket; // <- usa o socket do common

  // ===================== 1) Estado e seletores =====================
  let roomCode = null;
  let you = null;
  let stopTimer = null;
  let currentQ = null;
  ...
  btnJoin.addEventListener('click', () => {
    ...
    setRole('player'); // habilita resume no common.js
    const sessionId = localStorage.getItem('sessionId');

    ioSocket.emit('joinRoom', { roomCode, nick, avatar, sessionId });
  });

  ioSocket.on('joined', ({ room, you: me }) => {
    you = me;
  ...
  // ===================== 4) Perguntas / timer / resposta =====================
  ioSocket.on('newQuestion', ({ idx, total, timeLimitMs, q }) => {
    currentQ = q;
    ...
    if (q.type === 'processo_q1') {
      ...
      q.tokens.forEach(tok => {
        ...
        t.onclick = () => {
          if (t.classList.contains('used')) return; t.classList.add('used');
      ...
      function updateBtn(){
        const text = Array.from(build.querySelectorAll('.token')).map(x=>x.textContent).join(' ');
        sendAnswer.disabled = text.trim().length < 5;
      }
      sendAnswer.onclick = () => {
        const arr = Array.from(build.querySelectorAll('.token')).map(x=>x.textContent);
        const text = arr.join(' ');
        ioSocket.emit('submitAnswer', { roomCode, payload: { text, tokens: arr } });
        sendAnswer.disabled = true;
      };
    }
    ...
    if (q.type === 'processo_qMulti') {
      ...
      sendAnswer.onclick = () => {
        const sel = Array.from(picked.values());
        ioSocket.emit('submitAnswer', { roomCode, payload: { selected: sel } });
        sendAnswer.disabled = true;
      };
    }
    ...
    if (q.type === 'bio_abcd') {
      ...
      q.choices.forEach((opt,i)=>{
        ...
        el.onclick = () => {
          $all('.option').forEach(x=>x.classList.remove('selected'));
          el.classList.add('selected'); choice=i; sendAnswer.disabled=false;
        };
        grid.appendChild(el);
      });
      pQArea.appendChild(prompt); pQArea.appendChild(grid);
      sendAnswer.onclick = () => {
        ioSocket.emit('submitAnswer', { roomCode, payload: { choice } });
        sendAnswer.disabled = true;
      };
    }
  });
  ...
  // ===================== 5) Revelação / final =====================
  ioSocket.on('reveal', ({ correct }) => {
    if (currentQ?.type === 'bio_abcd' && typeof correct.correct === 'number') {
      const el = pQArea.querySelector(`.option[data-idx="${correct.correct}"]`);
      if (el) el.classList.add('correct');
    }
    if (currentQ?.type === 'processo_qMulti' && Array.isArray(correct.correct)) {
      correct.correct.forEach(i=>{
        const el = pQArea.querySelector(`.option[data-idx="${i}"]`);
        if (el) el.classList.add('correct');
      });
    }
    if (currentQ?.type === 'processo_q1' && correct.answerText) {
      const div = document.createElement('div'); div.className='answer-banner';
      div.innerHTML = `<strong>Resposta correta:</strong> ${correct.answerText}`;
      pQArea.appendChild(div);
    }
  });

  ioSocket.on('gameOver', ({ leaderboard }) => {
  ...
  // ===================== 6) Erros =====================
  ioSocket.on('errorMsg', (msg)=> alert(msg));
})();
