...
(() => {
  ...
  btnCreate.addEventListener('click', () => {
    ...
    setRole('host'); // evita resume do lado do host
    ioSocket.emit('createRoom', { gameType, nick });
  });

  ioSocket.on('roomCreated', ({ room }) => {
    roomCode = room.code;
  ...
  ioSocket.on('lobbyUpdate', (dto) => {
    ...
    dto.players.forEach(p => {
      ...
      li.textContent = `${p.nick} — ${p.score} pts${p.offline ? ' (offline)' : ''}`;
  ...
  btnStart.addEventListener('click', () => {
    ioSocket.emit('startGame', { roomCode });
    show(stage, true);
  ...
  // ===================== 6) Revelação / ranking parcial =====================
  btnReveal.addEventListener('click', () => {
    pendingRankOverlay = true;
    ioSocket.emit('revealAnswer', { roomCode });
  });
  ...
  btnContinue.addEventListener('click', () => {
    ...
    ioSocket.emit('nextQuestion', { roomCode });
  });

  // ===================== 7) Pódio / final =====================
  function renderFinalPodium(board){
    finalPodium.innerHTML = '';
    const top3 = [board[1], board[0], board[2]]; // 2º, 1º, 3º
